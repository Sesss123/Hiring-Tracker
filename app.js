...
/* ======================================================================
   ...
   the git history) — no server, plaintext passwords, and a page refresh
   was the only thing separating "data" from "gone". It now talks to a
   real Node/Express + MySQL backend (see /backend) for every entity
   below. KEYS.session is the one thing still kept in localStorage on
...
const api = {
  ...
  feedback: {
    ...
    submit: (interviewId, payload) => apiFetch(`/feedback/interviews/${interviewId}`, { method: "POST", body: payload }),
  },
  decisions: {
...
function JobCard({ job, navigate }) {
  // Used to also show a live applicant count here, read straight out of
  // KEYS.applicants — which worked only because every applicant record
  // was sitting in plain localStorage, readable by anyone on this public
  // page. GET /api/applicants now correctly requires staff login, so a
  // per-job applicant count is no longer something an anonymous visitor
  // can see here — that's the access-control fix working as intended,
...
// Shared by every public page that needs the open-jobs list (Home,
// Careers, JobCard's callers). Replaces the old synchronous
// load(KEYS.jobs) read with a real fetch to GET /api/jobs — that endpoint
// is deliberately public (no login) since candidates never need an
// account to browse or apply, same as before.
function useOpenJobs() {
  const [jobs, setJobs] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    let cancelled = false;
    api.jobs.list(false).then((data) => { if (!cancelled) setJobs(data); })
      .catch((err) => console.error("Could not load open jobs:", err))
...
function JobDetails({ jobId, navigate }) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(true);
    try {
      // The AI match score used to be computed here, client-side, then
      // stored as-is. It's now computed server-side (same analyzeMatch()
      // logic, see backend/src/matching.js) at the moment the applicant
      // record is created, so HR can't be shown a score the client made up
      // and never had to prove. The CV file itself still lives in this
      ...
      setErrors((x) => ({ ...x, submit: err.message || "Could not save your application. Please try again." }));
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (jobLoading) {
    return <div className="container page-section"><p className="muted-small">Loading role…</p></div>;
  }
...
function Login({ navigate, onLogin }) {
  ...
  // Used to compare email/password against KEYS.users in plaintext,
  // right here in the browser. Now calls POST /api/auth/login, which
  // checks a bcrypt hash server-side and returns a signed JWT — this
  // function no longer has any way to see anyone's password itself.
  async function handleSubmit(e) {
    e.preventDefault();
    setError("");
    setSubmitting(true);
...
function Register({ navigate, onLogin }) {
  ...
  // Same client-side checks as before (password length, required fields)
  // stay as a first line of defense, but the account itself — including
  // the duplicate-email check and the password hashing — is now created
  // by POST /api/auth/register on the server, not by pushing a plaintext
  // object into KEYS.users.
  async function handleSubmit(e) {
    e.preventDefault();
    setError("");
...
function ProfileDetail({ record, jobTitle, onClose, actions }) {
  ...
  return (
    <div className="card detail-panel">
      ...
      {/* CV QUALITY SCORE (Gemini rubric-based, separate from AI Match above) */}
      {record.qualityScore != null && (
        <div style={{ marginTop: "10px" }}>
          <strong>CV Quality Score:</strong>{" "}
          <span className="match-score">{record.qualityScore}%</span>
          {record.qualityBreakdown && (
            <p className="muted-small" style={{ marginTop: 4 }}>
              Education {record.qualityBreakdown.educationScore}/20 · Experience {record.qualityBreakdown.experienceScore}/25 ·
              {" "}Skills {record.qualityBreakdown.skillsScore}/20 · Certifications {record.qualityBreakdown.certificationScore}/10 ·
              {" "}Structure {record.qualityBreakdown.structureScore}/15 · Achievements {record.qualityBreakdown.achievementScore}/10
              {record.qualityBreakdown.reasoning && <><br />{record.qualityBreakdown.reasoning}</>}
            </p>
          )}
        </div>
      )}
...
function JobsTab() {
  ...
  // CREATE OR UPDATE JOB — now POST/PUT to the real backend instead of
  // splicing the array in localStorage. The client-side required-field
  // check stays as a first line of defense; the server enforces the same
  // rule again (see backend/src/routes/jobs.routes.js) since a browser
  // console could otherwise skip this check entirely.
  async function handleSubmit(e) {
    e.preventDefault();
    ...
    try {
      if (editingJobId) {
        await api.jobs.update(editingJobId, form);
  ...
  // CLOSE / REOPEN JOB — this button has no dedicated tracked backlog
  // item (see the project's own notes on Edit/Close/Reopen being dropped
  // from the tracked backlog on lecturer guidance) but it's real, working
  // code, so it gets a real endpoint: PATCH /api/jobs/:id/status.
  ...
  async function confirmDelete() {
    ...
    try {
      await api.jobs.remove(jobToDelete.id);
    } catch (err) {
      setError(err.message);
      setJobToDelete(null);
      return;
...
function ApplicantsTab() {
  ...
  const [qualityCheckingId, setQualityCheckingId] = useState(null);

  // Everything this tab needs — jobs, applicants, the interviewer
  // dropdown, and which applicants are already on the shortlist — now
  // comes from GET requests to the real backend instead of
  // load(KEYS.x)'s synchronous localStorage reads.
  async function refresh() {
    const applicantsData = await api.applicants.list();
    setApplicants(applicantsData);
  }
  async function refreshCandidateIds() {
    const shortlist = await api.candidates.list();
    setCandidateApplicantIds(shortlist.map((c) => c.applicantId));
  }
  useEffect(() => {
    api.jobs.list(true).then(setJobs).catch((err) => setActionError(err.message));
    api.users.list("interviewer").then(setInterviewers).catch((err) => setActionError(err.message));
  ...
  async function updateStatus(applicantId, status) {
    try {
      await api.applicants.setStatus(applicantId, status);
      await refresh();
    } catch (err) {
  ...
  // HR-triggered Gemini CV Quality Score (rubric-based: education,
  // experience relevance, skills overlap, certifications, structure,
  // achievements) — separate from AI Match (keyword similarity to the job).
  // Run per-applicant on demand rather than automatically on every apply,
  // since each check is a live Gemini API call.
  async function runQualityCheck(applicantId) {
    setQualityCheckingId(applicantId);
    setActionError("");
    try {
      await api.applicants.qualityCheck(applicantId);
      await refresh();
    } catch (err) {
      setActionError(err.message);
    } finally {
  ...
  function isAlreadyCandidate(applicantId) {
    return candidateApplicantIds.includes(applicantId);
  }

  async function addToCandidates(applicant) {
    try {
      if (isAlreadyCandidate(applicant.id)) return;
      await api.applicants.promote(applicant.id);
      await refreshCandidateIds();
      // CandidatesTab listens for this same event to pick up the new
      // shortlist entry without a page reload — unchanged from before.
  ...
  return (
    <div>
      ...
      <div className="table-wrap">
        <table className="data-table">
          <thead><tr><th>Name</th><th>Job</th><th>AI Match</th>{cvCompareJob && <th>CV Similarity</th>}<th>CV Quality</th><th>Pipeline</th><th>Status</th><th>CV</th><th>Actions</th></tr></thead>
          <tbody>
            {visible.map((a) => (
              <React.Fragment key={a.id}>
                <tr>
                  ...
                  <td>
                    {a.qualityScore != null ? (
                      <span className="match-score">{a.qualityScore}%</span>
                    ) : (
                      <button className="btn btn-outline btn-sm" disabled={qualityCheckingId === a.id} onClick={() => runQualityCheck(a.id)}>
                        {qualityCheckingId === a.id ? "Checking..." : "Run Quality Check"}
                      </button>
                    )}
                  </td>
...
function CandidatesTab() {
  ...
  async function togglePin(id) {
    try {
      await api.candidates.togglePin(id);
      await refresh();
    } catch (err) {
      setActionError(err.message);
    }
...
function InterviewerDashboard({ user }) {
  ...
  // backend/src/routes/interviews.routes.js) — the old client-side
  // .filter(i => i.interviewerId === user.id) is no longer needed here.
  async function refresh() {
    const [iv, apps, fb] = await Promise.all([api.interviews.list(), api.applicants.list(), api.feedback.list()]);
    setInterviews(iv);
    setCandidates(apps);
    setFeedbackAll(fb);
  }
  ...
  async function submitFeedback(iv) {
    try {
      await api.feedback.submit(iv.id, form);
      setOpenId(null);
      await refresh();
    } catch (err) {
      setError(err.message);
...
function ReviewCandidates({ user }) {
  ...
  const candidates = applicants.filter((c) => !jobFilter || c.jobId === jobFilter).sort((a, b) => b.matchScore - a.matchScore);
  function jobFor(id) { return jobs.find((j) => j.id === id); }
  const detail = detailId ? applicants.find((c) => c.id === detailId) : null;
  const interviewsForDetail = detail ? interviews.filter((i) => i.candidateId === detail.id) : [];
  function feedbackFor(interviewId) { return feedbackAll.find((f) => f.interviewId === interviewId); }
  function interviewerName(id) { const u = users.find((x) => x.id === id); return u ? u.name : "-"; }
...
function DecisionLog() {
  ...
  useEffect(() => {
    api.decisions.list().then(setDecisions).catch(console.error);
    api.applicants.list().then(setCandidates).catch(console.error);
    api.jobs.list(true).then(setJobs).catch(console.error);
    api.users.list().then(setUsers).catch(console.error);
  }, []);
